import { X, Save, RotateCcw, User, Settings as SettingsIcon, Database, Download, Upload } from 'lucide-react';
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        ...
        >
          ...
          <div className="p-6 space-y-8 max-h-[70vh] overflow-y-auto chat-scrollbar">
            ...
            <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
              <label className="flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-200">
                <Database size={18} className="text-orange-500" /> Manajemen Data (Backup & Restore)
              </label>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Simpan riwayat percakapan dan pengaturan Anda ke dalam file JSON, atau pulihkan dari backup sebelumnya.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <button 
                  onClick={onExportData}
                  className="flex flex-col items-center gap-2 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all group"
                >
                ...
                <label className="flex flex-col items-center gap-2 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all group cursor-pointer">
                  <Upload className="text-orange-500 group-hover:scale-110 transition-transform" />
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Import Restore</span>
                  <input 
                    type="file" 
                    accept=".json" 
                    className="hidden" 
